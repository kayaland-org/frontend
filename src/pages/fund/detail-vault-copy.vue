            ...
            <div class="fee-box" :class="showFee?'':'disable'">
              <p class="fee-title">
                <span class="head-name">
                  {{ $t('fundDetail.fundDetail_fee_title') }} <span> {{ $t('fundDetail.fundDetail_fee_description_1') }}</span>
                </span>
              </p>
              <div class="fee-main">
...
<!--                </i18n>-->
        ...
        <div class="detail-right" v-if="!showMobileBuy">
          ...
          <div class="fee-box" :class="showFee?'':'disable'">
            <p class="fee-title">
              <span class="head-name">{{ $t('fundDetail.fundDetail_fee_title') }} <span> {{ $t('fundDetail.fundDetail_fee_description_1') }}</span></span>
<!--              <svg-icon icon-class="icon_arrow_normal.png" :class-name="showGroupBuy?'icon rotate':'icon'"></svg-icon>-->
            </p>
            <div class="fee-main">
              <p class="fee-item" v-show="buyFee !== 0">
...
export default {
  ...
  methods: {
    ...
    checkAmout (price) {
      ...
      this.getBuyApproveState()
      this.fundContract.methods.calcUsdtToKf(new BigNumber(price).times(new BigNumber(10 ** this.useCurrency.decimals)).toFixed()).call().then(res => {
    ...
    getFundPrice (amount) {
      ...
      this.fundContract.methods.calcKfToUsdt(amount_).call().then(token => {
        if (amount) {
    ...
    getAum () {
      this.fundContract.methods.totalSupply().call().then(total => {
        this.fundContract.methods.calcKfToUsdt(total).call().then(res => {
          this.axios.get(`${globalConfig.coinGeckoBaseUrl}/simple/price`, {params: {ids: this.useCurrency.coinGeckoId, vs_currencies: 'usd'}}).then(data => {
            if (data.status === 200) {
    ...
    calcToWeth (val) {
      if (new BigNumber(val).comparedTo(0) === 0) {
        ...
        this.fundContract.methods.calcKfToUsdt(new BigNumber(val).times(new BigNumber(10 ** this.fund.decimals)).toFixed()).call().then(res => {
